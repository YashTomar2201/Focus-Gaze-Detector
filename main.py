...
# HARD LIMIT: Relative head angle max
# (Increased to 100 to allow full range of motion if eyes compensate)
HARD_HEAD_LIMIT = 100.0  

# COMPROMISE LIMITS:
# If you are within these angles relative to YOUR center, eyes can save you.
COMP_HEAD_LIMIT = 100.0  
COMP_PITCH_LIMIT = 45.0
...
while True:
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            if calibrated:
                # --- APPLY OFFSETS (This is where limits shift) ---
                ...
                # --- 7. OVERRIDES (COMPROMISE LOGIC) ---
                
                # Check 1: Horizontal Safety Range
                head_yaw_safe = abs(rel_head_yaw) < COMP_HEAD_LIMIT
                
                if head_yaw_safe:
                    # Left Comp: Head Left, Eyes Right (> 4)
                    ...
                    # Right Comp: Head Right, Eyes Left (< -4)
                    if (rel_head_yaw > 20 and eye_yaw_deg < -4):
                        is_distracted = False
                        override_text = "[H-Comp]"
                        distraction_start_time = None 
